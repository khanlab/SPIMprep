...
#        slurm_config["tmp"],
        ...
        str(log_out),
        "--export",
        f"ALL,SUBJECT={subject},ACQ={acq},STAIN_0={stain_0},STAIN_1={stain_1},STAIN_2={stain_2},INPUT_PATH={input_path},OUTPUT_BIDS_DIR={output_bids_dir},WORK_DIR={work_dir},CONDA_PREFIX={conda_prefix},RUN_PY_PATH={RUN_PY_PATH}",
    ]
...
@click.option(
    ...
    required=True,
    type=click.Path(),
    help="Path to BIDS output directory.",
)
@click.option(
    "--work-dir",
    required=True,
    type=click.Path(exists=True, dir_okay=True, file_okay=False, readable=True),
    help="Path to scratch or work directory.",
    default="/tmp",
)
@click.option(
    "--conda-prefix",
    required=True,
    type=click.Path(exists=True, dir_okay=True, file_okay=False, readable=True),
    help="Path to conda prefix for Snakemake.",
)
...
@click.option(
    "--dry-run", is_flag=True, help="Only print the sbatch commands without running."
)
@click.option(
    "--yes", "-y", is_flag=True, help="Skip confirmation before submitting jobs."
)
def main(
    samples_file,
    sbatch_script,
