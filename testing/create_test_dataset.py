...
def get_members_hybrid(
    source_dir, step, start_tile_x, start_tile_y, num_tiles_x, num_tiles_y
):
    """
    ...
    for member in members:
        ...
        if member_slice == 0:
            ...
            with tifffile.TiffFile(member.name) as tif:
                ...
                tile_files = meta_dict["OME"]["Image"]["ca:CustomAttributes"][
                    "TileConfiguration"
                ]["@TileConfiguration"].split("  ")[1:]
                proper_files = []
                ...
                meta_dict["OME"]["Image"]["ca:CustomAttributes"]["TileConfiguration"][
                    "@TileConfiguration"
                ] = new_tile_config
...
@app.command()
def create_test_subset_hybrid(
    path_to_source_tar: Annotated[
        str, typer.Argument(help="ex: dir1/dir2/dataset.tar")
    ],
    path_to_output_tar: Annotated[
        str, typer.Argument(help="ex: dir1/dir2/tes_dataset.tar")
    ],
    slice_step: int = 20,
    x_start: int = 0,
    num_x: int = 3,
    ...
    """
    members, wanted_slices, wanted_pairs = get_members_hybrid(
        path_to_source_tar, slice_step, x_start, y_start, num_x, num_y
    )
