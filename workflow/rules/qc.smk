...
"Generates an html file for comparing before and after flatfield correction"
    ...
    output:
        ...
        corr_images_dir=directory('qc/sub-{subject}_sample-{sample}_acq-{acq}/images/corr'),
        uncorr_images_dir=directory('qc/sub-{subject}_sample-{sample}_acq-{acq}/images/uncorr'),
    log:
        bids(
            root="logs",
...
"Generates an html file to view the volume rendered image"
    input:
        ome=get_input_ome_zarr_to_nii(),
    output:
        resources=directory("qc/sub-{subject}_sample-{sample}_acq-{acq}/volume_resources"),
