...
for chunk, (tile_corr, tile_uncorr) in enumerate(zip(proc_data[0], unproc_data[0])):
    channels = []
    for chan_num, (channel_corr, channel_uncorr) in enumerate(
        zip(tile_corr, tile_uncorr)
    ):
        slice = ff_s_start
        slices = []
        while slice < len(channel_corr):
            ...
            # Save images
            plt.imsave(
                corr_image_dir + "/" + corr_image_name, clipped_data_corr, cmap=ff_cmap
            )
            plt.imsave(
                uncorr_image_dir + "/" + uncorr_image_name,
                clipped_data_uncorr,
                cmap=ff_cmap,
            )
            ...
            image = {
                "slice": slice,
                "img_corr": corrected_img_path,
                "img_uncorr": uncorrected_img_path,
            }
            slices.append(image)
            slice += ff_s_step
