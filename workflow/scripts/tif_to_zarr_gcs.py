...
gcsfs_opts = {
    "project": snakemake.params.storage_provider_settings["gcs"].get_settings().project,
...
def build_zstack(gcs_uris, fs, size_z, size_y, size_x, dtype):
    ...
    lazy_arrays = [dask.delayed(read_tiff_slice)(fs, uri) for uri in gcs_uris]

    # Convert the list of delayed objects into a Dask array
    return da.stack(
        [
            da.from_delayed(lazy_array, shape=(size_y, size_x), dtype=dtype)
...
def build_zstack_from_single(gcs_uri, zstack_metadata, fs):
    ...
    lazy_arrays = [dask.delayed(read_tiff_slice)(fs, gcs_uri, page) for page in pages]
    ...
    return da.stack(
        [
            da.from_delayed(
                lazy_array,
                shape=zstack_metadata["shape"],
                dtype=zstack_metadata["dtype"],
            )
            for lazy_array in lazy_arrays
        ],
        axis=0,
...
if is_tiled:
    ...
    tiles = []
    for i_tile, (tilex, tiley) in enumerate(
        product(metadata["tiles_x"], metadata["tiles_y"])
    ):
        ...
        for i_chan, channel in enumerate(metadata["channels"]):
            ...
            if is_zstack:
                ...
                tif_file = in_tif_pattern.format(
                    tilex=tilex,
                    tiley=tiley,
                ...
                zstacks.append(
                    da.from_delayed(
                        delayed(read_stack_as_numpy)(
                            "gcs://" + tif_file, fs, size_z, size_y, size_x
                        ),
                        shape=(size_z, size_y, size_x),
                        dtype="uint16",
                    ).rechunk((1, size_y, size_x))
                )
