...
for i_chan, channel in enumerate(metadata["channels"]):
    for i_tile in metadata["chunks"]:
        ...
        affine = np.eye(3, 4)
        affine[0, 3] = metadata["lookup_tile_offset_x"][key] / float(
            metadata["physical_size_x"]
        )
        affine[1, 3] = -metadata["lookup_tile_offset_y"][key] / float(
            metadata["physical_size_y"]
        )
        affine[2, 3] = -metadata["lookup_tile_offset_z"][key] / float(
        ...
        bdv_writer.append_view(
            ...
            calibration=(
                1,
                1,
                float(metadata["physical_size_z"]) / float(metadata["physical_size_x"]),
            ),
        )
...
update_xml_h5_to_n5(
    snakemake.params.temp_xml, snakemake.output.bdv_xml, snakemake.output.bdv_n5
)
...
for setup_i, ((chan_i, chan), (tile_i, tile)) in enumerate(
    product(enumerate(metadata["channels"]), enumerate(metadata["chunks"]))
):
    ...
    for ds in range(max_downsampling_layers):
        ...
        with ProgressBar():
            zstack.to_zarr(
                n5_store,
                component=f"setup{setup_i}/timepoint0/s{ds}",
                overwrite=True,
                compute=True,
            )
        # add attributes for downsampling
        a = zarr.open_array(
            store=n5_store, path=f"setup{setup_i}/timepoint0/s{ds}", mode="r+"
        )
