...
for subdir in sorted(tiles_dir.iterdir()):
    ...
    if zarr_path.is_dir():
        ...
        multiscale = from_ngff_zarr(str(zarr_path))  # Ensure string path if required by the library
...
with ProgressBar():
    stacked.to_zarr(snakemake.output.zarr, overwrite=True)  # Convert to string if API expects string
